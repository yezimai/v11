            ...
            if int(v[0]) == 2:  # 虚拟机,获取虚拟机的ip，责任人信息
                ...
                try:
                    ...
                    dblog.error("[ERROR] Query error, Catch exception:[ %s ], file: [ %s ], line: [ %s ]" \
                                % (e, __file__, sys._getframe().f_lineno))
                if len(data) == 0:
                    print('**virtualserver ip fail or no permissions...')
                    return []
                for i in range(len(data)):
                    ...
                    dic1['server_type'] = data[i][3]
                    dic1['server_id'] = data[i][4]

                    # print data[i][1]
                    r_list.append(dic1)
                # print('<---vitual--->\033[34;1m%s\033[0m' %r_list)
            else:   # 如果是物理服务器
                ...
                for i in range(len(data)):
                    ...
                    dic2['server_type'] = data[i][3]
                    dic2['server_id'] = data[i][4]
                    r_list.append(dic2)
                    # print('5555555555555physicalserver',dic2)
