...
class Operation_dbModelClass(BaseDbModelClass):
    ...
    def inserToLog(self, username, ip, action, url,memo):
        '''用户名，ip，动作，内容'''
        sql = '''
            INSERT into operation_log (`username`,`ip`,`action`,`url`,`memo`) 
            VALUES(%s,%s,%s,%s,%s)
        '''
        try:
            status = self._cursorInsert(sql, [username, ip, action, url, memo])
